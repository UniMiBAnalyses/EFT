...
by varying the luminosity (EFT 6th-order operator: Q_HW), using the ptl2 distribution
from the ntuple ntuple_RcHW_0p3_2.root (with a fixed background, x_0 = 100 GeV).

c++ -o createFilesPTL2_lum createFilesPTL2_lum.cpp `root-config --glibs --cflags` 
...
int main (int argc, char** argv) 
{
	...
	vector<TFile*> files;
	TFile* f1 = new TFile ("ptl2_lum_1.root","recreate");
	TFile* f2 = new TFile ("ptl2_lum_2.root","recreate");
	TFile* f3 = new TFile ("ptl2_lum_3.root","recreate");
	TFile* f4 = new TFile ("ptl2_lum_4.root","recreate");
	TFile* f5 = new TFile ("ptl2_lum_5.root","recreate");
	TFile* f6 = new TFile ("ptl2_lum_6.root","recreate");
	TFile* f7 = new TFile ("ptl2_lum_7.root","recreate");
	TFile* f8 = new TFile ("ptl2_lum_8.root","recreate");
	TFile* f9 = new TFile ("ptl2_lum_9.root","recreate");
	TFile* f10 = new TFile ("ptl2_lum_10.root","recreate");
	TFile* f11 = new TFile ("ptl2_lum_11.root","recreate");
	...
	//mins and max of ptl2 distribution 
	float min = 20;
	float max = 600;
	float first_limit = 380; //from here bin_width*2
	float second_limit = 430; //from here bin_width*4
	float RMS = 31.7565; //RMS of ptl2 distribution
	...
	float luminosity_array[] = {100, 200, 400, 800, 1600, 3200, 6400, 12800, 25600, 51200};
	vector<TH1F> histos[10];
	float integrals[10][3];
	
	float x_0 = 100; //fixed value for the parameter x_0 in the exponential model of the background
	...
	for (int ntuple_number = 0; ntuple_number < 3; ntuple_number++) // sm, int, bsm
	{
		...
		for (int lum_index = 0; lum_index < 10; lum_index++)
		{
	...
	for (int lum_index = 0; lum_index < 10; lum_index++) 
	{
		...
		/*cout << "lum_"<< lum_index + 1 << "--------------------------------------------" << endl
		     << integrals[lum_index][0] << "\t" << integrals[lum_index][1] << "\t" << integrals[lum_index][2] << "\t" << bkg->Integral() << endl;*/


		//PLOTS
		TH1F* histo_sum = new TH1F(histos[lum_index][0] + cHW*histos[lum_index][1] + cHW*cHW*histos[lum_index][2]);
		...
		h_stack->Draw("hist");
		stringstream ss_cHW, ss_lum;
		ss_lum << luminosity_array[lum_index];
		ss_cHW << cHW;
		string title = "c_{HW} = " + ss_cHW.str() + ", x_{0} = 100 GeV, L = " + ss_lum.str() + " fb^{-1}";
		...
		string name_png = "ptl2_lum_" + ss_name_png.str() + ".png";
