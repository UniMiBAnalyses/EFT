...
int main (int argc, char** argv) 
{
	...
	string x_axis[13]   = {"met","m_{jj}","m_{ll}","p_{tl1}","p_{tl2}","p_{tj1}","p_{tj2}",
	...
	//for cHW
	range_file.open("range_cHW.txt", ios::in);
	...
	int i = 1;
	while (i != 15) //14 raws in the file (13 variables + "no shape" distribution)
	{
		if (i < 14) {
		...
		else range_file >> sx_95_noshape >> sx_68_noshape >> dx_68_noshape >> dx_95_noshape >> min;
	...
	float range_68_sorted[13];
	float range_95_sorted[13];
	string x_axis_sorted[13];
	...
	cnv->SetGrid();
	TH1F *h1 = new TH1F("h1","Widths of confidence ranges",13,0,13);
	...
	TH1F *h2 = new TH1F("h2","h2",13,0,13);
	h2->SetFillColor(kRed-7);
	h2->SetBarWidth(0.08);
	h2->SetBarOffset(0.55);
	h2->SetStats(0);
	for (i=1;i<=13;i++) h2->SetBinContent(i, range_95_sorted[i-1]);
	h2->Draw("b same");

	float range_68_noshape = dx_68_noshape - sx_68_noshape;
	float range_95_noshape = dx_95_noshape - sx_95_noshape;

	TLine* line_68 = new TLine(0, range_68_noshape, 13, range_68_noshape);
	line_68->SetLineColor(kBlue);
	line_68->SetLineWidth(2.);
	line_68->Draw("same");

	TLine* line_95 = new TLine(0, range_95_noshape, 13, range_95_noshape);
	line_95->SetLineColor(kRed);
	line_95->SetLineWidth(2.);
	line_95->Draw("same");

	TLegend* legend = new TLegend(0.13,0.42,0.41,0.59);
	legend->AddEntry(h1,"68% confidence range","f");
	legend->AddEntry(h2,"95% confidence range","f");
	legend->AddEntry(line_68,"68% confidence range (limit)","l");
	legend->AddEntry(line_95,"95% confidence range (limit)","l");
	legend->Draw();

	//To save the plot
	//cnv->Print("range_vs_kineticvar_cW.png", "png");
	//cnv->Print("range_vs_kineticvar_cHW.png", "png");
