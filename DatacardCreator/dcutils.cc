  ...
  for (int iVar = 0 ; iVar < allvars.size () ; ++iVar)
    {
      ...
      treeReaderValues.insert (
        pair<string, TTreeReaderValue<float> > (
          "etaj1",
          TTreeReaderValue<float> (reader, "etaj1")
        )
      ) ;
      treeReaderValues.insert (
        pair<string, TTreeReaderValue<float> > (
          "etaj2",
          TTreeReaderValue<float> (reader, "etaj2")
        )
    ) ;

     // treeReaderValues["etaj2"] = TTreeReaderValue<float> (reader, "etaj2") ;
    }
  ...
  while (reader.Next ())
    {
      ...
      //adding some cuts
      /*if((*(treeReaderValues.at ("etaj1")) > 5) || (*(treeReaderValues.at ("etaj2")) > 5) || abs( *(treeReaderValues.at ("etaj1")) - *(treeReaderValues.at ("etaj2")) )<2.5){
          save = false ;
          break ;
      }*/

      if((*etaj1 > 5) || (*etaj2>5) || abs( *etaj1-*etaj2 )<2.5){
          save = false ;
      }
